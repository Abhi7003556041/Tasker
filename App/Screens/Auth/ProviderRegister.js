...
const ProviderRegister = props => {
  ...
  console.log('reeeegister data>>>', registerData)
  ...
  // console.log("IMMMMMMMMMMM",image)
  ...
  const proregister = async () => {
    ...
    let data = {
      ...
      "curpID": curpID,
      "identificationNumber": govtIDNumber,
      "aboutMe": about,
      "deviceToken": token,
    ...
    AuthService.provProfileUpdate(service != "" ? data : dataN)
      .then(res => {
        console.log('LONGres>>>>>>>>>>>>>>>', res);
        // Alert.alert('res=========== ',res)
        if (res.status) {
          HomeService.requesterProfileUpdate(data1).then(res => {
          });
          dispatch(setuser({ ...registerData, image: image,AllUData:data }))
          AuthService.setAccount({ ...registerData, image: image,AllUData:data });
  ...
  const changeAddress = (desc) => {
    ...
    Geocoder.from(desc)
      .then(a => {
        // console.log('desc-----------------',a.results[0]?.geometry?.location?.lat,a.results[0]?.geometry?.location?.lng)
